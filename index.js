    ...
    for (let i = 0; i < ps.length; i++) {
        ...
        if (!p.hasyinxiao == true) {
            ...
            p.addEventListener("click", async function (event) {
                let head = getRequestHeaders(token);
                //console.log("getContext",await getContext().getCharacterCardFields(getContext().characterId));
                console.log("getContext2",getContext());
                ...
                if (targetElement.nodeType === Node.ELEMENT_NODE) {
                    if (event.detail === 2) { // 二击触发
                        if (window.isImmersiveSoundPreparing) {
                            ...
                            console.log('st-immersive-sound is busy.');
                            return;
                        }
                        ...
                        if (marker && (marker.isPlaying || isSfxWaitPlaying()) && marker.element === event.currentTarget) {
                            stopAllAudio(marker);
                            marker = null;
                            return;
                        }
                        ...
                        if (bgmlist) {
                            const categories = ["Music", "Ambiance", "SFX", "SFX_WAIT"];
                            let lastMusicEnd = -1; 
                            for (const category of categories) {
                                if (bgmlist.hasOwnProperty(category)) {
                                    for (let i = 0; i < bgmlist[category].length; i++) {
                                        ...
                                        console.log("bgmItem.src", bgmItem.src);
